            ...
            if len(det):
                ...
                for j, (*xyxy, conf, cls) in enumerate(reversed(det[:, :6])):
                    ...
                    if save_img or save_crop or view_img:  # Add bbox to image
                        ...
                        print(f'{p.stem}_{j} ', *[int(i.numpy()) for i in xyxy], flush=True)
                        with open(f'{p.stem}_{j}_crop.txt', 'a') as f:
                            f.write(*[int(i.numpy()) for i in xyxy])
                    if save_crop:
                        # begin g0nx0s
                        #if len(det[:, :6]) > 1:
                        ...
                        save_one_box([0,0,cpu_mask.shape[1],cpu_mask.shape[0]], imc_masked, file=save_dir / 'crops' / names[c] / f'{p.stem}_{j}.jpg', BGR=True)
